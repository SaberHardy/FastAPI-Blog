...
async def lifespan(_app: FastAPI):
    ...
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)  # create the database if not exist
    yield
    # shutdown
...
# -> connects routers to our app, adding the prefix is /api/users - ex: /api/users/user/{user_id}
...
async def home(request: Request, db: Annotated[AsyncSession, Depends(get_db)]):
    result = await db.execute(select(models.Post).options(selectinload(models.Post.author))
                              .order_by(desc(models.Post.date_posted)))
...
# @app.get("/api/posts/{post_id}", response_model=list[PostResponse])
...
#
#     existing_user = db.execute(select(models.User)
...
async def post_page(request: Request, post_id: int,
                    db: Annotated[AsyncSession, Depends(get_db)]):  # get single post from the page
...
async def general_http_exception_handler(request: Request, exception: StarletteHTTPException):
    ...
    message = (exception.detail if exception.detail else "An error occurred on the server side.")
